<template>
    <div>
        <el-header>
            <Header
                    active-index="/Followupvisit"></Header>
        </el-header>
        <el-main>
<!--                三个表格 今日待随访/超期未随访/全部未随访        -->
            <el-tabs v-model="activeName" @tab-click="handleClick" stretch>
                <el-tab-pane label="今日待随访" name="first">
                    ...
                    <el-table
                            :data="todayUndoSelect.slice((todayUndoCurrentPage-1)*todayUndoPageSize,todayUndoCurrentPage*todayUndoPageSize)"
                            style="width: 100%">
                        <el-table-column
                                label="病历号"
                                prop="caseId">
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope" class="table-operate">
                                ...
                                <el-button
                                        size="mini"
                                        type="success"
                                        @click="addFollowup(scope.$index, scope.row)">新增随访</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                    <!--                                            表格分页-->
                ...
                <el-tab-pane label="超期未随访" name="second">
                    <el-table
                            :data="overdueSelect.slice((overdueCurrentPage-1)*overduePageSize,overdueCurrentPage*overduePageSize)"
                            style="width: 100%">
                        <el-table-column
                                label="病历号"
                                prop="caseId">
                ...
                <el-tab-pane label="全部未随访" name="third">
                    <el-table
                            :data="undoSelect.slice((undoCurrentPage-1)*undoPageSize,undoCurrentPage*undoPageSize)"
                            style="width: 100%">
                        <el-table-column
                                label="病历号"
                                prop="caseId">
            ...
            <el-dialog :title="currentPatientName + '基本信息'" :visible.sync="dialogFormVisible" width="30%">
                <el-form :model="infoForm">
                    ...
                    <el-form-item label="患者单位" :label-width="formLabelWidth">
                        <el-input v-model="infoForm.unit" autocomplete="off" placeholder="请输入患者单位" disabled></el-input>
                    </el-form-item>
            ...
            <!--                            新增随访          -->
            <el-dialog :title="currentPatientName +'添加随访'" :visible.sync="dialogFormVisibleFollowup" width="30%">
                <el-form :model="addFollow">
                    <el-form-item label="预防计划" :label-width="formLabelWidth">
                        <el-select v-model="addFollow.visitPlan" placeholder="请选择随访类型">
                    ...
                    <el-form-item label="是否随访成功" :label-width="formLabelWidth">
                        <el-radio v-model="addFollow.visitResult" label="true" @change="followupResultChange(addFollow)">是</el-radio>
                        <el-radio v-model="addFollow.visitResult" label="false" @change="followupResultChange(addFollow)">否</el-radio>
                    </el-form-item>
                    <el-form-item label="随访内容" :label-width="formLabelWidth" v-if="visitresult">
                        <el-input v-model="addFollow.visitContent" autocomplete="off"></el-input>
                    </el-form-item>
                    <div v-else>
                        <el-form-item label="随访未成功原因" :label-width="formLabelWidth">
                            <el-input v-model="addFollow.visitRemark" autocomplete="off"></el-input>
                        </el-form-item>
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button @click="dialogFormVisibleFollowup = false">取 消</el-button>
                    <el-button type="primary" @click="submitEditShortInfo('addFollow')">确 定</el-button>
                </div>
            </el-dialog>
        </el-main>
...
<script>
    ...
    export default {
        ...
        created() {
            const _this = this
            //          超期的随访
            _this.$axios.get("/followup/Overdue" , {
                headers: {
            ...
            //          今日的随访
            _this.$axios.get("/followup/todayUndo" , {
                headers: {
                    "Authorization": _this.$store.getters.getToken
                }
            }).then(res => {
        ...
        methods: {
            ...
            todayUndoDataSizeChange() {
                this.todayUndoSelect = this.todayUndo.filter(data => !this.search
            ...
            overdueDataSizeChange() {
                this.overdueSelect = this.overdue.filter(data => !this.search
                    || data.patientId.toLowerCase().includes(this.search.toLowerCase())
                    || data.patientName.toLowerCase().includes(this.search.toLowerCase())
                );
            },
            ...
            //
            undoDataSizeChange() {
                this.undoSelect = this.undo.filter(data => !this.search
                    || data.patientId.toLowerCase().includes(this.search.toLowerCase())
                    || data.patientName.toLowerCase().includes(this.search.toLowerCase())
                );
            },
            ...
            patientPastCase(index, row) {
                const _this = this
                _this.$router.push({name: "PostCaseDetail", params: {id: row.caseId}})
            },
            submitEditShortInfo() {
                console.log(this.addFollow);
                const _this = this
                _this.$axios.post("/followup/editFollowup/", _this.addFollow, {
                    headers: {
                        "Authorization": _this.$store.getters.getToken
                    }
                }).then(res => {
